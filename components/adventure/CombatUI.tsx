...
export default function CombatUI({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
      <div className="backdrop-blur-[10px] bg-[rgba(15,23,42,0.6)] border border-red-500/50 rounded-[10px] max-w-2xl w-full max-h-[85vh] overflow-y-auto flex flex-col">
        ...
        <div className="p-6 space-y-5 flex-1">
          ...
          {combatState.enemy && !combatState.won && !combatState.fled && (
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className="py-3 bg-red-500/20 border border-red-500/50 text-red-400 rounded-[10px] hover:bg-red-500/30 disabled:opacity-50 font-bold"
              >
                Attaquer
              </button>
              <button
                onClick={onDefend}
                disabled={combatState.turn !== "player"}
                className="py-3 bg-blue-500/20 border border-blue-500/50 text-blue-400 rounded-[10px] hover:bg-blue-500/30 disabled:opacity-50"
              >
                Défense
              </button>
              <button
                onClick={onFlee}
                disabled={combatState.turn !== "player"}
                className="py-3 bg-yellow-500/20 border border-yellow-500/50 text-yellow-400 rounded-[10px] hover:bg-yellow-500/30 disabled:opacity-50"
              >
                Fuir
              </button>
            </div>
          ...
          {combatState.enemy && !combatState.won && !combatState.fled && (
            <div className="mt-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {getAbilitiesForClass(character?.classe || "guerrier").map(
                  (ability) => {
                    const inCooldown =
          ...
          {(combatState.won || combatState.fled) && (
            <button
              onClick={onEnd}
              className="w-full py-3 bg-gradient-to-r from-[#06b6d4] to-[#3b82f6] text-white rounded-[10px] font-bold hover:opacity-90 transition-opacity"
            >
              {combatState.won
