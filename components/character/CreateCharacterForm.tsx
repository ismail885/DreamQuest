...
export default function CreateCharacterForm({
  ...
  const statBars = [
    {
      key: "force",
      label: "Force",
      value: classInfo.baseStats.force,
      icon: Swords,
    },
    {
      key: "agility",
      label: "Agilite",
      value: classInfo.baseStats.agility,
      icon: Zap,
    },
    {
      key: "magie",
      label: "Intelligence",
      value: classInfo.baseStats.magie,
      icon: Brain,
    },
    {
      key: "endurance",
      label: "Endurance",
      value: classInfo.baseStats.endurance,
      icon: Heart,
    },
  ];
  ...
  const handleNext = () => {
    if (currentStep < classes.length - 1) setCurrentStep(currentStep + 1);
  };
          ...
          <div className="flex flex-col md:flex-row gap-6 mb-8">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span
                  className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium border ${diffCfg.bg} ${diffCfg.color} ${diffCfg.border}`}
                >
              ...
              <div className="flex flex-wrap gap-2">
                {abilitiesWithInfo.map((ability, idx) => {
                  const badge =
                    ABILITY_BADGES[ability.type] ?? ABILITY_BADGES.PASSIVE;
                  return (
                    <div key={ability.name} className="group relative">
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 p-3 rounded-lg bg-gray-900 border border-gray-700 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none">
                        <span
                          className={`inline-block px-2 py-0.5 rounded text-xs font-medium mb-2 border ${badge.color} ${badge.border}`}
                        >
                          {badge.label}
                        </span>
            ...
            {characterName.trim().length >= 3 && (
              <div className="mb-5 p-3 bg-cyan-900/10 border border-cyan-500/20 rounded-lg">
                <p className="text-sm text-gray-400 text-center">
                  ...
                  &mdash; <span className="text-white ">{classType}</span>
                </p>
              </div>
            )}
