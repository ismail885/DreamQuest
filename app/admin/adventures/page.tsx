  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white dark:text-gray-900">Gestion des aventures</h1>
          <p className="text-gray-400 dark:text-gray-500 mt-2">{totalCount} aventure{totalCount !== 1 ? "s" : ""} disponible{totalCount !== 1 ? "s" : ""}</p>
        </div>
        <a
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-900/50 dark:bg-gray-100/50">
              <tr>
                <th className="px-6 py-4 text-left text-gray-400 dark:text-gray-500 font-medium text-sm">Titre</th>
                <th className="px-6 py-4 text-left text-gray-400 dark:text-gray-500 font-medium text-sm">Description</th>
                <th className="px-6 py-4 text-left text-gray-400 dark:text-gray-500 font-medium text-sm">Popularité</th>
                <th className="px-6 py-4 text-left text-gray-400 dark:text-gray-500 font-medium text-sm">Création</th>
            ...
            <tbody className="divide-y divide-gray-800 dark:divide-gray-200">
              {loading ? (
                ...
                adventures.map((adventure) => (
                  <tr key={adventure.id} className="hover:bg-gray-800/30 dark:hover:bg-gray-100/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-purple-500/20 flex items-center justify-center">
                          <Star className="w-5 h-5 text-purple-400" />
                        </div>
                        <span className="text-white dark:text-gray-900 font-medium">{adventure.titre}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-400 dark:text-gray-500 max-w-xs truncate">
                      {adventure.description || "Sans description"}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <ThumbsUp className="w-4 h-4 text-amber-400" />
                        <span className="text-white dark:text-gray-900 font-medium">{adventure.popularite}</span>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0f1623] dark:bg-white border border-gray-800 dark:border-gray-200 rounded-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-gray-800 dark:border-gray-200">
              <h2 className="text-xl font-bold text-white dark:text-gray-900">{viewAdventure.titre}</h2>
              <button onClick={() => setViewAdventure(null)} className="text-gray-400 dark:text-gray-500 hover:text-white dark:hover:text-gray-900">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="text-white dark:text-gray-900 mt-1">{viewAdventure.description || "Aucune description"}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-gray-400 dark:text-gray-500 text-sm">Popularité</label>
                ...
                <div>
                  <label className="text-gray-400 dark:text-gray-500 text-sm">Date de création</label>
                  <p className="text-white dark:text-gray-900 mt-1 flex items-center gap-2">
              ...
              {viewAdventure.auteur_id && (
                <div>
                  <label className="text-gray-400 dark:text-gray-500 text-sm">ID Auteur</label>
                  <p className="text-white dark:text-gray-900 mt-1">#{viewAdventure.auteur_id}</p>
                </div>
              )}
            </div>
