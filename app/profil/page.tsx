...
export default function ProfilPage() {
  ...
  useEffect(() => {
    ...
    // Premier chargement → spinner, sinon refresh silencieux
    if (!dataLoadedRef.current) {
      dataLoadedRef.current = true;
      setLoading(true);
      loadUserData(user.id).finally(() => setLoading(false));
    } else {
      loadUserData(user.id);
    }

    const onFocus = () => { if (user?.id) loadUserData(user.id); };
    const onVisibility = () => { if (!document.hidden && user?.id) loadUserData(user.id); };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [authLoading, user?.id, activeTab, router]);
    ...
    <div className="min-h-screen bg-[#0a0e1a] dark:bg-gray-50 text-white dark:text-gray-900 flex flex-col">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <div className="lg:w-80 flex-shrink-0">
              <div className="bg-[#0f1623] dark:bg-white border border-gray-700/50 dark:border-gray-300 rounded-xl lg:rounded-2xl p-4 md:p-6">
                <div className="flex flex-col items-center mb-4 md:mb-6">
                  ...
                  <h2 className="text-lg md:text-xl font-bold text-white dark:text-gray-900">{userProfile?.nom_utilisateur || "Aventurier"}</h2>
                  <p className="text-gray-400 dark:text-gray-500 text-sm">Niveau {currentLevel} • Rang +{Math.floor(currentLevel * 3 + stats.likes / 100)}</p>
                ...
                <div className="mb-6">
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-gray-400 dark:text-gray-500">Expérience</span>
                    <span className="text-red-400 font-semibold">{currentExperience.toLocaleString()} / {maxExperience.toLocaleString()}</span>
                  </div>
                                        <div className="h-2 bg-gray-700 dark:bg-gray-200 rounded-full overflow-hidden">
                    <div 
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="text-center p-2 bg-gray-800/30 dark:bg-gray-100/30 rounded-lg">
                    <div className="text-2xl font-bold text-cyan-400">{stats.storiesPlayed}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500">Histoires</div>
                  </div>
                  <div className="text-center p-2 bg-gray-800/30 dark:bg-gray-100/30 rounded-lg">
                    <div className="text-2xl font-bold text-cyan-400">{stats.storiesCreated}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500">Créées</div>
                  </div>
                  <div className="text-center p-2 bg-gray-800/30 dark:bg-gray-100/30 rounded-lg">
                    <div className="text-2xl font-bold text-cyan-400">{stats.likes}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500">Likes</div>
                  </div>
                  <div className="text-center p-2 bg-gray-800/30 dark:bg-gray-100/30 rounded-lg">
                    <div className="text-2xl font-bold text-cyan-400">{stats.trophies}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500">Trophées</div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <button 
                    ...
                    className="w-full py-3 px-4 bg-[#1a2332] dark:bg-gray-200 border border-gray-600/50 rounded-lg text-white dark:text-gray-900 font-medium hover:bg-[#1a2332] transition-colors flex items-center justify-center gap-2"
                  >
            ...
            <div className="flex-1">
              <div className="bg-[#0f1623] dark:bg-white border border-gray-700/50 dark:border-gray-300 rounded-2xl overflow-hidden">
                <div className="flex overflow-x-auto flex-nowrap border-b border-gray-700/50 dark:border-gray-300 bg-[#0a0e1a] dark:bg-gray-50 scrollbar-thin">
                  <button
                    onClick={() => setActiveTab("stories")}
                    className={`flex-shrink-0 py-4 px-6 text-sm font-medium transition-all ${
                      activeTab === "stories"
                        ? "bg-cyan-500/10 text-cyan-400 border-b-2 border-cyan-400"
                        : "text-gray-400 dark:text-gray-500 hover:text-white dark:hover:text-gray-900 hover:bg-gray-700/20 dark:hover:bg-gray-200/20"
                    }`}
                  >
                    Mes Histoires
                  </button>
                    ...
                    <div className="space-y-4">
                      {userSaves.length > 0 ? (
                        userSaves.map((save) => (
                          <div
                            key={save.id}
                            className="bg-[#0f1623] dark:bg-white border border-gray-700/30 dark:border-gray-300/30 rounded-xl p-5 hover:border-cyan-500/30 transition-all cursor-pointer"
                          >
                            <div className="flex justify-between items-start mb-3">
                              <h3 className="text-lg font-semibold text-white dark:text-gray-900">{save.aventure_titre}</h3>
                              {getStatusBadge(save.status)}
                            </div>
                            <div className="mb-2">
                              <span className="text-sm text-gray-400 dark:text-gray-500">Progression : {save.progression}%</span>
                            </div>
                            <div className="h-2 bg-gray-700 dark:bg-gray-200 rounded-full overflow-hidden">
                              <div
                        ...
                        <div className="text-center py-12">
                          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gray-700/50 dark:bg-gray-200/50 flex items-center justify-center">
                          ...
                          <h3 className="text-lg font-semibold text-white dark:text-gray-900 mb-2">Aucune histoire jouée</h3>
                          <p className="text-gray-400 dark:text-gray-500 mb-4">Commencez une aventure pour voir votre progression ici.</p>
                          <button 
                      ...
                      {userAchievements ? (
                        <>
                          <div className="text-center mb-6">
                            ...
                            <div className="text-gray-400 dark:text-gray-500 text-sm">réalisations débloquées</div>
                          </div>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {userAchievements.achievements.map((achievement) => {
                              const iconName = achievement.icon;
                              const iconClass = `w-6 h-6 ${achievement.unlocked ? "text-cyan-400" : "text-gray-500 dark:text-gray-600"}`;
                              return (
                                <div
                                  key={achievement.id}
                                  className={`p-4 rounded-xl border ${
                                    achievement.unlocked
                                      ...
                                      : "bg-gray-800/30 dark:bg-gray-100/30 border-gray-700/30 dark:border-gray-300/30 opacity-50"
                                  }`}
                                >
                                  <div className="mb-2">
                                  ...
                                  <h4 className={`font-semibold ${achievement.unlocked ? "text-white dark:text-gray-900" : "text-gray-500 dark:text-gray-600"}`}>
                                    {achievement.title}
                                  </h4>
                                  <p className={`text-xs ${achievement.unlocked ? "text-gray-400 dark:text-gray-500" : "text-gray-600"}`}>
                        ...
                        <div className="text-center py-12">
                          ...
                          <h3 className="text-lg font-semibold text-white dark:text-gray-900 mb-2">Aucune réalisation</h3>
                          <p className="text-gray-400 dark:text-gray-500">Vos trophées et badges apparaîtront ici quand vous les débloquerez.</p>
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <div className="space-y-4">
                      {userCreations.length > 0 ? (
                        userCreations.map((creation) => (
                          ...
                          >
                            <div className="flex justify-between items-start">
                              <h3 className="text-lg font-semibold text-white dark:text-gray-900">{creation.titre}</h3>
                        ...
                        <div className="text-center py-12">
                          ...
                          <h3 className="text-lg font-semibold text-white dark:text-gray-900 mb-2">Aucune création</h3>
                          <p className="text-gray-400 dark:text-gray-500 mb-4">Créez votre première aventure pour la voir apparaître ici.</p>
                          <button 
                  ...
                  {activeTab === "quests" && (
                    <div className="space-y-4">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold text-white dark:text-gray-900">Quêtes du jour</h3>
                      ...
                      {dailyQuests.length > 0 ? (
                            ...
                            <div
                              ...
                              className={`p-4 rounded-xl border ${
                                quest.completed
                                  ...
                                  : "bg-[#0f1623] dark:bg-white border-gray-700/30 dark:border-gray-300/30"
                              }`}
                            >
                              <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  <h4 className={`font-semibold ${quest.completed ? "text-green-400" : "text-white dark:text-gray-900"}`}>
                                    {quest.title}
                                  </h4>
                                </div>
                                <span className="text-yellow-400 text-sm">+{quest.xpReward} XP</span>
                              </div>
                              <p className="text-gray-400 dark:text-gray-500 text-sm mb-2">{quest.description}</p>
                              <div className="flex justify-between text-xs text-gray-500 dark:text-gray-600 mb-1">
                        ...
                        <div className="text-center py-12">
                          <p className="text-gray-400 dark:text-gray-500">Chargement des quêtes...</p>
                        </div>
                      )}
                    </div>
                  )}
                      ...
                      {userCharacters.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {userCharacters.map((char, index) => {
                            ...
                            <div
                              key={char.id ?? index}
                              className="bg-gray-900/50 dark:bg-gray-50/50 backdrop-blur-sm border border-gray-800 dark:border-gray-200 hover:border-cyan-400 rounded-lg overflow-hidden transition-all duration-300 group"
                            >
                              <div className="relative h-32 bg-gradient-to-b from-gray-800 dark:from-gray-200 to-gray-900 dark:to-gray-100">
                                <div className="absolute inset-0 bg-gradient-to-t from-gray-900 dark:from-gray-100 via-transparent to-transparent" />
                                ...
                                <div className="absolute bottom-2 left-2 right-2">
                                  ...
                                  <div className="h-1 bg-gray-900/80 dark:bg-gray-100/80 rounded-full overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-cyan-500 to-blue-500" style={{ width: `${xpPercent}%` }} />
                                  </div>
                                </div>
                              </div>
                              <div className="p-4">
                                <h3 className="text-lg font-bold text-white dark:text-gray-900 mb-1">{char.nom_personnage}</h3>
                                ...
                                {passif && (
                                  <div className="mb-3 p-2 bg-cyan-500/10 border border-cyan-500/30 rounded-lg">
                                    ...
                                    <span className="text-xs text-gray-400 dark:text-gray-500 block">{passif.desc}</span>
                                  </div>
                                )}
                                <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-600">
                        ...
                        <div className="text-center py-12">
                          ...
                          <h3 className="text-lg font-semibold text-white dark:text-gray-900 mb-2">Aucun personnage</h3>
                          <p className="text-gray-400 dark:text-gray-500 mb-4">Créez votre premier personnage pour commencer l&apos;aventure.</p>
                          <button
                            onClick={() => router.push("/create-character")}
                            className="px-6 py-2 bg-cyan-500 hover:bg-cyan-600 text-white font-medium rounded-lg transition-colors"
                          >
                      ...
                      {userCharacters.length > 0 ? (
                        <div className="space-y-6">
                          {userCharacters.map((char, index) => {
                            ...
                            return (
                              <div key={char.id ?? index} className="bg-[#0f1623] dark:bg-white border border-gray-700/30 dark:border-gray-300/30 rounded-xl p-5">
                                <div className="flex items-center gap-4 mb-4">
                                  ...
                                  <div>
                                    <h3 className="text-lg font-semibold text-white dark:text-gray-900">{char.nom_personnage}</h3>
                                ...
                                <div className="mb-4">
                                  ...
                                  <div className="h-3 bg-gray-700 dark:bg-gray-200 rounded-full overflow-hidden">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                  {Object.entries(stats).map(([stat, value]) => {
                                    ...
                                    return (
                                      <div key={stat} className="bg-gray-800/50 dark:bg-gray-100/50 rounded-lg p-3">
                                        <div className="flex justify-between text-sm mb-1">
                  ...
                  <div className="h-2 bg-gray-700 dark:bg-gray-200 rounded-full overflow-hidden">
                                ...
                                {passif && (
                                  <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-lg p-4">
                                    ...
                                    <span className="text-gray-400 dark:text-gray-500 text-sm">{passif.desc}</span>
                                  </div>
                                )}
                              </div>
                            );
                        ...
                        <div className="text-center py-12">
                          ...
                          <h3 className="text-lg font-semibold text-white dark:text-gray-900 mb-2">Aucun personnage</h3>
                          <p className="text-gray-400 dark:text-gray-500 mb-4">Créez votre premier personnage pour voir son évolution.</p>
                          <button
                            onClick={() => router.push("/create-character")}
                            className="px-6 py-2 bg-cyan-500 hover:bg-cyan-600 text-white font-medium rounded-lg transition-colors"
                          >
      ...
      {isSettingsModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
          ...
          <div className="relative bg-[#0f1623] dark:bg-white border border-gray-700/50 dark:border-gray-300 rounded-t-2xl md:rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl shadow-cyan-500/10 max-h-[85vh] overflow-y-auto md:max-h-none">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={closeSettingsModal}
                className="p-2 hover:bg-gray-700/50 dark:hover:bg-gray-200/50 rounded-lg transition-colors"
              >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-[#1a2332] dark:bg-gray-200 rounded-lg">
                <div className="flex items-center gap-3">
                  <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                  </svg>
                  <span className="text-white dark:text-gray-900">Notifications</span>
                </div>
                <button
              ...
              <div className="flex items-center justify-between p-4 bg-[#1a2332] dark:bg-gray-200 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white dark:text-gray-900">Mode sombre</span>
                </div>
                <button
                  onClick={toggleTheme}
              ...
              <div className="p-4 bg-[#1a2332] dark:bg-gray-200 rounded-lg">
                <div className="flex items-center gap-3 mb-3">
                  <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" />
                  </svg>
                  <span className="text-white dark:text-gray-900">Langue</span>
                </div>
                <select
                  value={language}
                  onChange={(e) => setLanguage(e.target.value)}
                  className="w-full px-4 py-2 bg-[#0f1623] dark:bg-white border border-content-secondary/30 rounded-lg text-white dark:text-gray-900 focus:outline-none focus:border-primary transition-colors"
                >
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={closeSettingsModal}
                className="flex-1 py-3 px-4 bg-[#1a2332] dark:bg-gray-200 border border-gray-600/50 rounded-lg text-white dark:text-gray-900 font-medium hover:bg-[#1a2332] hover:text-white dark:hover:text-gray-900 transition-colors"
              >
                Annuler
              </button>
              <button
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
          ...
          <div className="relative bg-[#0f1623] dark:bg-white border border-gray-700/50 dark:border-gray-300 rounded-t-2xl md:rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl shadow-cyan-500/10 max-h-[85vh] overflow-y-auto md:max-h-none">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-white dark:text-gray-900 mb-2">
                  Nom d&apos;utilisateur
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 bg-[#1a2332] dark:bg-gray-200 border border-gray-600/50 rounded-lg text-white dark:text-gray-900 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors"
